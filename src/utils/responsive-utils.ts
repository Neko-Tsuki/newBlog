...
export const getResponsiveSidebarConfig = () => {
	...
	return {
		...
		// 检查各设备上左右侧边栏是否有可见组件
		hasLeftComponentsMobile: isBothSidebars && widgetManager.hasVisibleComponentsInSidebar("left", "mobile"),
		hasLeftComponentsTablet: isBothSidebars && widgetManager.hasVisibleComponentsInSidebar("left", "tablet"),
		hasLeftComponentsDesktop: isBothSidebars && widgetManager.hasVisibleComponentsInSidebar("left", "desktop"),
		hasRightComponentsMobile: isBothSidebars && widgetManager.hasVisibleComponentsInSidebar("right", "mobile"),
		hasRightComponentsTablet: isBothSidebars && widgetManager.hasVisibleComponentsInSidebar("right", "tablet"),
		hasRightComponentsDesktop: isBothSidebars && widgetManager.hasVisibleComponentsInSidebar("right", "desktop"),
	};
};
...
export const generateGridClasses = (config: ReturnType<typeof getResponsiveSidebarConfig>) => {
	...
	if (isBothSidebars) {
		...
		if (mobileShowSidebar && hasLeftComponentsMobile && hasRightComponentsMobile) {
			mobileGrid = "grid-cols-1";
		} else if (mobileShowSidebar && (hasLeftComponentsMobile || hasRightComponentsMobile)) {
			mobileGrid = "grid-cols-1";
		}
		
		// 平板端
		let tabletGrid = "md:grid-cols-1";
		if (tabletShowSidebar && hasLeftComponentsTablet && hasRightComponentsTablet) {
		...
		if (desktopShowSidebar && hasLeftComponentsDesktop && hasRightComponentsDesktop) {
...
export const generateRightSidebarClasses = (config: ReturnType<typeof getResponsiveSidebarConfig>) => {
	...
	// 根据是否有左侧边栏决定列位置
	const tabletCol = hasLeftComponentsTablet ? "md:col-start-3 md:col-end-4" : "md:col-start-2 md:col-end-3";
	const desktopCol = hasLeftComponentsDesktop ? "lg:col-start-3 lg:col-end-4" : "lg:col-start-2 lg:col-end-3";
	
	// 根据是否有可见组件决定显示
	const mobileDisplay = mobileShowSidebar && hasRightComponentsMobile ? "block" : "hidden";
	const tabletDisplay = tabletShowSidebar && hasRightComponentsTablet ? `md:block md:row-start-1 md:row-end-2 md:max-w-[17.5rem] ${tabletCol}` : "md:hidden";
	const desktopDisplay = desktopShowSidebar && hasRightComponentsDesktop ? `lg:block lg:row-start-1 lg:row-end-2 lg:max-w-[17.5rem] ${desktopCol}` : "lg:hidden";
